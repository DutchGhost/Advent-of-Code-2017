...
#[inline(always)]
pub fn noop<T: ?Sized>(_: &mut T) {}

pub fn to_vec_of_nums<S: AsRef<str>, N: FromStr>(input: S) -> Vec<N>
where
    <N as FromStr>::Err: ::std::fmt::Debug
{
    input
        .as_ref()
        .lines()
        .map(|line| line.split_whitespace().map(|num| num.parse().unwrap()).collect::<Vec<N>>())
        .flat_map(|v| v)
        .collect()
}
...
impl Spiral {
    ...
    /// Returns a generator that generates a spiral. Takes any updater F, where F provides the new value
    /// based on the current position and current value.
